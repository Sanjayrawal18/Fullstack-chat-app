    ...
    <div className='h-full container mx-auto px-4 pt-20 max-w-5xl'>
      <div className='space-y-6'>
        <div className="flex flex-col gap-1">
          <h2 className='tex-lg font-semibold'>Theme</h2>
          <p className='text-sm text-base-content/70'>Choose a theme for your chat interface</p>
        </div>
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
          {THEMES.map((t, index) => (
            <button key={index} className={`group flex flex-col items-center gap-1 5 p-2 rounded-lg transition-colors ${theme === t ? "bg-base-200" : "hover:bg-base-200/50"}`} onClick={() => setTheme(t)}>
                ...
                <div className="px-4 py-3 border-b border-base-300 bg-base-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="tex-xs text-base-content/70">Online</p>
                    </div>
                  </div>
                </div>
